...
- **Count blocks.** A blank `qtd_*` entry becomes zero when another
  count in the same vehicle or injury-severity block is populated.
  Entirely blank blocks remain missing.
...
- **Coordinates.** Coordinate pairs outside a validation bounding box
  around São Paulo state become `NA`. This catches extreme malformed
  values and `(0, 0)` placeholders but is a rectangular screen, not a
  state-boundary test. No rows are dropped; typed and raw modes preserve
