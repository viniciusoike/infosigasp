...
- `processing = "clean"` starts from the typed import, then trims text,
  ...
  `ano_mes_*`, converts crash-type flags to logical, fills blank `qtd_*`
  entries with zero inside populated count blocks, converts
  `tempo_sinistro_obito` to integer, removes a trailing `".0"` from
  `numero_logradouro`, and validates coordinate pairs against a bounding
  box around Sao Paulo state.
