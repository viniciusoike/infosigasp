# Authors and Citation

## Authors

- **Vinicius Oike**. Author, maintainer, copyright holder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/viniciusoike/infosigasp/blob/data-2026-06/DESCRIPTION)

Oike V (2026). *infosigasp: Download and Import Traffic Crash Data from
INFOSIGA-SP*. R package version 0.1.0,
<https://github.com/viniciusoike/infosigasp>.

    @Manual{,
      title = {infosigasp: Download and Import Traffic Crash Data from INFOSIGA-SP},
      author = {Vinicius Oike},
      year = {2026},
      note = {R package version 0.1.0},
      url = {https://github.com/viniciusoike/infosigasp},
    }
